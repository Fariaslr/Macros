...
public class RealizaConsulta {
    ...
    public void fazendoConsulta() {
        ...
        } while (validaInteiro.isHeight(consulta.getAltura()) == false);
        ...
        } while (validaInteiro.isActivity(consulta.getNivelAtividadeFisica()) == false);

        consulta.determinaDieta(plano.getObjetivo());

        do {
            System.out.printf("Número de refeições: ");
            consulta.setNumeroRefeicoes(scanf.nextInt());
        } while (validaInteiro.isNumberOfMeals(consulta.getNumeroRefeicoes()) == false);
    }
}
